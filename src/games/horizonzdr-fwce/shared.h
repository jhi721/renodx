...
#define HORIZON_TONE_MAP_TYPE_CUSTOMIZED 2.f
#define HORIZON_TONE_MAP_TYPE_PSYCHOV 3.f
...
struct ShaderInjectData {
  ...
  float color_grade_blowout;               // 0..1, default 0; drives grade dechroma (luminance-keyed)
  ...
  float peak_ratio;           // max(peak / max(diffuse, 1), 1.001)
  float internal_peak_ratio;  // peak_ratio pushed into the internal pre-gamma domain (Idiom-A)
};
