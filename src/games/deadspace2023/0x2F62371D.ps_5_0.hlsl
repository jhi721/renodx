// ---- Created with 3Dmigoto v1.3.16 on Tue Jun 02 09:18:23 2026
// RenoDX: Dead Space (2023) fused display node 0x2F62371D — native scRGB HDR retrofit.
// Vanilla logic preserved verbatim (HDR grade bit1 + UI composite bit8 + SDR bit16 + LUT bit2);
// only the final unbounded `*1.25` output is replaced with paper-white + per-channel highlight
// roll-off -> linear scRGB. See FinalizeDeadSpace() in shared.h.
#include "./shared.h"
  ...
  out float4 o0 : SV_Target0)
{
  ...
  // RenoDX: deferred UI composite (Vanilla+). The vanilla bit8 path mixes the HUD into the scene
  // BEFORE the grade/roll-off; we instead capture it here and composite after FinalizeDeadSpace at
  // a user-controlled UI Brightness, so the HUD stays crisp and independent of Game Brightness.
  float3 dsUiColor = float3(0, 0, 0);
  float dsSceneAlpha = 1.0;
  bool dsDeferUI = false;
  ...
  if (r1.x != 0) {
    ...
    if (shader_injection.tone_map_mode == TONE_MAP_MODE_VANILLA) {
      // Vanilla: composite UI into the scene pre-grade exactly as the game does.
      r2.xyz = cb0[3].yyy * r2.xyz;
      r0.w = -r2.w * cb0[3].z + 1;
      r0.xyz = r0.xyz * r0.www + r2.xyz;
    } else {
      // Vanilla+: defer the HUD past the roll-off. t1 is treated as linear (the game composites it
      // into linear scene*100), so we re-apply only the alpha here and scale to nits at the end.
      dsUiColor = r2.xyz;
      dsSceneAlpha = -r2.w * cb0[3].z + 1.0;
      dsDeferUI = true;
    }
  }
  if (r1.y != 0) {
    ...
    r2.xyz = cmp(float3(3.13080018e-005,3.13080018e-005,3.13080018e-005) >= r2.xyz);
  ...
  // RenoDX canonical output: vanilla analytic grade (r0, 0.18 = mid-gray, 1.0 = 100-nit diffuse ref)
  // -> mode-driven RenoDRT tone map -> intermediate -> deferred HUD composite -> scRGB SwapChainPass.
  // The library handles all nits scaling (tonemapped * diffuse_white_nits / 80, clamped at peak).
  float3 color = ToneMapDeadSpace(r0.xyz);
  // Perceptual film grain (Vanilla+ only), on the tonemapped scene before the HUD composite.
  if (shader_injection.tone_map_mode == TONE_MAP_MODE_VANILLA_PLUS) {
    color = ApplyFilmGrainDeadSpace(color, v1.xy);
  }
  color = renodx::draw::RenderIntermediatePass(color);
  // Deferred HUD (SDR / Vanilla+): composite in the graphics-white-normalized intermediate space so
  // SwapChainPass lands it at Graphics White (UI Brightness). Vanilla composited it pre-grade above.
  if (dsDeferUI) {
    color = color * dsSceneAlpha + dsUiColor;
  }
  o0.xyz = renodx::draw::SwapChainPass(color);
  o0.w = 1;
  return;
}