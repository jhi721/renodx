...
// HDR tone maps of the analytic scene permutations. Include after the permutation declares its _Globals.
...
float3 MELEToneMapAnalytic(float3 graded, float3 scene, float3 bloom, float3 vignette_tint) {
...
float3 MELEToneMapME3Analytic(float3 graded, float3 untonemapped, float3 vignette_tint) {
  float q;
  float3 proxy;
  ...
  const float3 work = MELENativeLinear(MELEGradeME3Analytic(proxy), GammaColorScaleAndInverse.xyz, true) / q;
